...
export interface RelatorioCusteioResponse {
  data: OperacaoNavio[];
  pagination: {
    current_page: number;
    per_page: number;
...
export interface RelatorioCusteioParams {
  page?: number;
  limit?: number;
  search?: string;
  id_pessoa?: number;
...
export const relatorioCusteioService = {
  async getResultados(params: RelatorioCusteioParams = {}): Promise<RelatorioCusteioResponse> {
    ...
    const response = await httpClient.get<RelatorioCusteioResponse>(`/relatorio-custeio?${searchParams.toString()}`);
    return response;
  },

  async getDetalhes(id: number): Promise<DetalhesOperacao> {
    const response = await httpClient.get<DetalhesOperacao>(`/relatorio-custeio/${id}`);
    return response;
  }
};
