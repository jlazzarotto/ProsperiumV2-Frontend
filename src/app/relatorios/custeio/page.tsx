...
import {
  relatorioCusteioService,
  type OperacaoNavio,
  type TotaisNavio,
  type RelatorioCusteioParams,
  type DetalhesOperacao
} from "@/app/services/relatorio-custeio-service"
import { formatCurrency, formatDate } from "@/lib/utils"
import customToast from "@/components/ui/custom-toast"
import type { BusinessUnit, Person } from "@/types/types"

export default function RelatorioCusteioPage() {
  ...
  const loadData = async () => {
    setLoading(true)
    try {
      const params: RelatorioCusteioParams = {
        page: currentPage,
        limit: itemsPerPage
      }
      ...
      const response = await relatorioCusteioService.getResultados(params)
  ...
  const toggleRowExpansion = async (idOperacao: number) => {
    ...
    if (expandedRows.has(idOperacao)) {
      ...
      if (!detalhesOperacoes[idOperacao] && !loadingDetalhes.has(idOperacao)) {
        ...
        try {
          const detalhes = await relatorioCusteioService.getDetalhes(idOperacao)
      ...
      <div className="w-full py-6 px-4">
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold tracking-tight">Relatório de Custeio</h1>
              <p className="text-muted-foreground">
                Visualize apenas os lançamentos marcados como custeio por operação de navio
              </p>
            </div>
          </div>
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                <CardTitle>Operações com Custeio</CardTitle>
                                ...
                                <div className="p-4">
                                  {loadingDetalhes.has(operacao.id_operacao) ? (
                                    ...
                                    <div className="space-y-4">
                                      ...
                                      <div>
                                        <h5 className="font-medium mb-2">Lançamentos de Custeio</h5>
